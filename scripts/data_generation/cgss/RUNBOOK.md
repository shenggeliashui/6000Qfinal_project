...
- **`--test_csv`**：须含 `input_prompt` / `output_dist` / `ordinal`；若 `test_ratio=0` 导致空表，请重新 `prepare_finetuning_data` 划出 test，或改用 `cgss-eval` 等含 test 行的 CSV。
- **`--loss_function_type`**：与训练 `ce` / `wd` 一致；终端打印按 batch 平均的 loss / KL / WD。
- 与 `train_utils.evaluation` 同一套选项头（` A`…` Z`）与 KL/WD 定义；**无需 vLLM**。
- **对比**：对 `eval_baseline.csv` 与 `eval_ft.csv` 的 `kl`、`wd` 做均值差、配对检验或逐题差分；**KL/WD 越低通常越好**（越接近 `output_dist`）。
