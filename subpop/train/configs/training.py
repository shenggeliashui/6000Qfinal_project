...
@dataclass
class train_config:
    model_name: str="PATH/TO/MODEL/NAME"
    ...
    run_test: bool=False # whether to run test when best validation loss is found
    ...
    gamma: float= 0.85 # multiplicatively decay the learning rate each step, so that per epoch LR is multiplied by gamma
    warmup_ratio: float=0.1 # ratio of total steps to warmup to the total number of steps
    ...
    dist_checkpoint_root_folder: str="PATH/TO/FULL/FINETUNING/CHECKPOINT" # will be used if using FSDP
    dist_checkpoint_folder: str="fine-tuned" # will be used if using FSDP
    save_optimizer: bool=True # will be used if using FSDP
    use_fast_kernels: bool = False # Enable using SDPA from PyTroch Accelerated Transformers, make use Flash Attention and Xformer memory-efficient kernels
    use_wandb: bool = True # Enable wandb for experiment tracking
